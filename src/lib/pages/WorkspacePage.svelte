...
import MarkdownDiff from "../components/MarkdownDiff.svelte";
import Toast from "../components/Toast.svelte";
import ErrorPage from "../components/ErrorPage.svelte";
import IpToolkit from "../components/IpToolkit.svelte";
import LauncherPage from "../components/LauncherPage.svelte";
import ToolLauncher from "../components/ToolLauncher.svelte";
import { activeEntries, canMoveEntry, orderedChildren } from "../domain/tree";
import {
  type Asset,
  cloneWorkspace,
  type EntryKind,
...
import { countMarkdownCharacters } from "../markdown/character-count";
...
const workspaceSession = createWorkspaceSession({
  ...
  setRepository: (next) => (repository = next),
  getAssetUrls: () => assetUrls,
  setAssetUrls: (next) => (assetUrls = next),
  setActiveEntryId: (next) => (activeEntryId = next),
  setExpanded: (next) => (expanded = next),
  setStatus: (next) => (status = next),
  setStatusTone: (next) => (statusTone = next),
  notify,
  saveNow,
});
...
onMount(() => {
  const boot = isLauncher || isIpToolkit || isNotFound
    ? undefined
    : window.setTimeout(() => void workspaceSession.initialize(), 0);
  const onKey = (event: KeyboardEvent) => {
    ...
    if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "k") {
      event.preventDefault();
      if (!event.shiftKey) {
        if (isLauncher) toolLauncher?.focusSearch();
        else openLauncher();
        return;
      }
  ...
  const refreshOnFocus = () => void workspaceSession.refreshFromStorage();
...
$effect(() => {
  ...
  void import("../components/DiagramEditor.svelte").then((module) => {
...
async function switchWorkspace(): Promise<void> {
  ...
  textInputRequest = {
    ...
    submitLabel: "開く",
    onSubmit: (selected) => void workspaceSession.openWorkspace(selected),
  };
}
...
async function createWorkspaceWithName(name: string): Promise<void> {
  ...
  try {
    ...
    assetUrls = {};
    workspaceSession.announceSave(created.id);
...
async function saveNow(): Promise<boolean> {
  ...
  try {
    ...
    workspaceSession.announceSave(saved.id);
    status = "保存済み";
    statusTone = "info";
    return true;
  } catch (error) {
...
async function saveDiagram(
  diagram: import("../domain/workspace").DiagramDocument,
): Promise<boolean> {
  if (
    !workspace ||
    !repository ||
...
async function restore(file: File): Promise<void> {
  ...
  try {
    ...
    if (!(await saveNow())) return;
    await workspaceSession.hydrateAssets();
...
function openLauncher(): void {
  toolLauncher?.openLauncher();
}

function closeLauncher(): void {
  toolLauncher?.closeLauncher();
}
...
<main class:document-import-page={isDocumentImport} class="app-shell">
  ...
  {#if isDocumentImport}
    <DocumentImportPanel
      ...
      onOpenDocument={(entry) => window.location.assign(`/workspace?entry=${encodeURIComponent(entry.id)}`)}
      onError={notify}
    />
  {:else}
...
{/if}
<ToolLauncher bind:this={toolLauncher} bind:open={launcherOpen} />
