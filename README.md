...
図表では、処理、判断、開始/終了、コンポーネント、データベース、キュー、注釈と AWS サービスを配置できます。各ノードの上下左右から自由に接続でき、接続線は曲線・直交・直線、片方向・双方向・矢印なし、実線・破線、ラベルを選べます。フロー、システムアーキテクチャ、AWS Web、AWS サーバーレス、ER のテンプレートも利用できます。

AWS ノードは AWS 公式アーキテクチャアイコンを使用し、生成する SVG の中にアイコンを埋め込みます。そのため Markdown への挿入、オフライン表示、ZIP バックアップと復元でも外部 CDN は不要です。ライブラリ選定の根拠とアイコン利用条件は [ADR 0002](docs/adr/0002-diagram-editor-and-aws-icons.md) を参照してください。
